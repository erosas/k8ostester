...
| `core/session.py` | interactive sessions (`k8ost session`): deploy the config, load as a scalable Deployment of self-contained loadgen pods (pod count = the load knob), faults on demand via a command queue; no timeline, no verdict — the human is the experiment plan. `--attach <ns>` skips deploy entirely: the driver auto-detects the technology (`detects()` classmethod), faults/telemetry/load run against the existing cluster, and teardown removes only k8ost artifacts — never the namespace |
