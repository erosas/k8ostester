...
> **Superseded by D19 (2026-07-07):** the observability tier was built, verified, and then
> removed — metrics are app-perspective/journal-only. Kept below as originally planned.
...
- ~~Grafana is AGPL~~ **Resolved twice over:** Perses replaced Grafana, then the whole
  observability tier was removed (D19) — metrics are journal-only, so the license question
  is moot.
