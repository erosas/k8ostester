...
def render(runs: list[dict], title: str, out: Path) -> Path:
    ...
    html = _TEMPLATE.replace("__TITLE__", title).replace("__PAYLOAD__", payload)
    out.write_text(html)
    return out
...
// goal matrix
{
  ...
  runs.forEach((r, i) => {
    ...
    key.className = 'k'; key.style.cssText =
      `display:inline-block;width:12px;height:2px;margin-right:6px;vertical-align:middle;background:var(${COLORS[i]})`;
    th.appendChild(key);
...
// legends
for (const id of ['legend1', 'legend2']) {
  const el = document.getElementById(id);
  runs.forEach((r, i) => {
    const s = document.createElement('span');
    const key = document.createElement('i');
    key.style.background = `var(${COLORS[i]})`;
...
function chart(mountId, field, yLabel) {
  ...
  const xMax = Math.max(...runs.map(r => r.series.at(-1).s)) + 1;
  const yMax = Math.max(...runs.flatMap(r => r.series.map(p => p[field] ?? 0))) * 1.1 || 1;
  ...
  runs.forEach((r, i) => {
    ...
    const d = pts.map((p, j) => `${j ? 'L' : 'M'}${x(p.s).toFixed(1)},${y(p[field]).toFixed(1)}`).join('');
    const path = document.createElementNS(ns, 'path');
    path.setAttribute('d', d);
    path.setAttribute('fill', 'none');
    path.setAttribute('stroke', css(COLORS[i]));
  ...
  svg.addEventListener('pointermove', e => {
    ...
    runs.forEach((r, i) => {
      ...
      k.className = 'k'; k.style.background = css(COLORS[i]);
      row.appendChild(k);
