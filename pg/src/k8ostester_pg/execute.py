...
def execute(k8s: ClusterClient, namespace: str, action_id: str, snapshot: dict) -> str:
    ...
    return handler(k8s, namespace, snapshot)


def _kill_primary(k8s: ClusterClient, ns: str, s: dict) -> str:
    chaos.kill_pod(k8s, ns, s["primary"])
    return f"killed primary {s['primary']}"


def _partition_primary(k8s: ClusterClient, ns: str, s: dict) -> str:
    chaos.partition_pod(k8s, ns, s["primary"])
    return f"partitioned primary {s['primary']}"


def _kill_replica(k8s: ClusterClient, ns: str, s: dict) -> str:
